...
class TestGetApi:
    ...
    @pytest.mark.asyncio
    async def test_double_checked_lock_returns_existing_api(self):
        """Race: _api set between outer check and lock acquisition -> inner check returns it."""
        sentinel = MagicMock(name="existing_api")

        original_lock = pool_module._init_lock

        async def _set_api_then_lock():
            # Simulate another coroutine that set _api just before us acquiring the lock
            pool_module._api = sentinel
            # Yield so the inner check fires after we enter the lock body
            return sentinel

        # Pre-set _api to None so the outer check passes, then set it inside the lock
        # to exercise line 36 (double-check inside async with).
        pool_module._api = None
        ...
        class _FakeLock:
            async def __aenter__(self_inner):
                ...
                if not acquired_once:
                    acquired_once = True
                    # Between outer `if _api is not None` and acquiring the lock,
                    # another task has set _api.
                    pool_module._api = sentinel
                return self_inner
